...
export async function destroy(id){  
    ...
    if (resp.status !== 204) return {error: "Erro ao apagar o cadastro selecionado. " + resp.status}
...
export async function update(cadastro){
    const updateURL = url + "/" + cadastro.id
    ...
    if (resp.status !== 200) return {error: "Erro ao atualizar o cadastro. " + resp.status}

    revalidatePath("/")
}

export async function getCadastros(id){
    const getUrl = url + "/" + id
    const resp = await fetch(getUrl)
    if (resp.status !== 200) return {error: "Erro ao buscar dados do cadastro. " + resp.status}
    const json = await resp.json()
    return json
}
